    ...
    switch (handleReply.type) {
      case "reply": {
        ...
        if (s.length == 0) {
          ...
          for (let ad of idad) {
            api.sendMessage({
              body: "[📄]→ Phản hồi từ " + name + ":\n" + (event.body) || "chỉ có tệp không có nội dung trả lời" + "\n----------------\n『 "+ global.config.ICON +" 』➜ Kèm theo tệp!", attachment: l, mentions: [{
                id: event.senderID,
                tag: name
              }]
            }, ad, (e, data) => global.client.handleReply.push({
      ...
      case "calladmin": {
        if (s.length == 0) {
          ...
          api.sendMessage({ body: `『 ${global.config.ICON} 』➜ Phản hồi từ admin ${name} đến bạn:\n--------\n${(event.body) || "chỉ có tệp không có nội dung trả lời"}\n--------\n[💬]→ Phản hồi tin nhắn này để tiếp tục gửi báo cáo về admin\n--------\n『 ${global.config.ICON} 』➜ Tệp admin gửi đến bạn!`, attachment: l, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    api.sendMessage(
      "=== 『 Success 』 ===\n▱▱▱▱▱▱▱▱▱▱▱▱▱\n『 "+ global.config.ICON +" 』➜  Đã gửi nội dung tới " + soad + " admin bot...",
      event.threadID,
      () => {
        var idad = global.config.ADMINBOT;
        if (s.length == 0) {
          for (let ad of idad) {
            api.sendMessage({ body: `『 ${global.config.ICON} 』➜  Báo cáo từ: ${name}\n『 ${global.config.ICON} 』→ 
 Box: ${namethread}\n『 ${global.config.ICON} 』➜ ID box: ${idbox}\n----------------\n『 ${global.config.ICON} 』➜ Nội dung: ${args.join(
              " "
            )}\n----------------\n『 ${global.config.ICON} 』➜ Time: ${gio}`, mentions: [{ id: event.senderID, tag: name }] },
              ad, (error, info) =>
          ...
          for (let ad of idad) {
            api.sendMessage({
              body: `『 ${global.config.ICON} 』➜ Báo cáo từ: ${name}\n⚡Box: ${namethread}\n『 ${global.config.ICON} 』➜ ID box: ${idbox}\n----------------\n『 ${global.config.ICON} 』➜ Lỗi: ${(args.join(
                " "
              )) || "chỉ có tệp không có nội dung báo cáo!"}\n----------------\n『 ${global.config.ICON} 』➜ Time: ${gio}\n----------------\n『 ${global.config.ICON} 』➜ Kèm theo tệp!`, attachment: l, mentions: [{ id: event.senderID, tag: name }]
            },
              ad, (error, info) =>
              global.client.handleReply.push({
                name: this.config.name,
