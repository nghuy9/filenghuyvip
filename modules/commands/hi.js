  ...
  if (KEY.includes(event.body.toLowerCase()) !== false) {
    ...
    mentions.push({
      ...
      id: event.senderID
        ...
        axios.get('https://webapi.chaocacbannhe.repl.co/loil').then(res => {
	      let ext = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);
	      let callback = function () {
         api.sendMessage({
                body: `❤️✨『 𝐀𝐔𝐓𝐎𝐍𝐎𝐓𝐈 』✨❤️\n━━━━━━━━━━━━━\n『 ${global.config.ICON} 』 ➜ 𝐇𝐞𝐥𝐥𝐨 ${name} 𝐜𝐮𝐭𝐢 𝐩𝐡𝐨̂ 𝐦𝐚𝐢 𝐪𝐮𝐞\n『 🌱 』 ➜𝐜𝐡𝐮́𝐜 𝐛𝐚̣𝐧 𝐦𝐨̣̂𝐭 𝐛𝐮𝐨̂̉𝐢 ${session} ${text}\n[⏰] 𝐓𝐡𝐨̛̀𝐢 𝐠𝐢𝐚𝐧: ${thu} ${gio}`,
                 attachment: fs.createReadStream(__dirname + `/cache/hi.${ext}`)
					        }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/hi.${ext}`), 
                 event.messageID);
				        };
				request(res.data.data).pipe(fs.createWriteStream(__dirname + `/cache/hi.${ext}`)).on("close", callback);
			})
